(function (global, factory) {
    ...
    var ChordComponent = function (_React$Component) {
        _inherits(ChordComponent, _React$Component);

        function ChordComponent(props) {
            _classCallCheck(this, ChordComponent);

            var _this = _possibleConstructorReturn(this, (ChordComponent.__proto__ || Object.getPrototypeOf(ChordComponent)).call(this, props));

            _this.defaultSize = 3;
            _this.renderOnLoad = true;
            _this.MUTED = -1;

            //               |pos < 10   |pos > 10, needs seperator                | Optional fingerings|Optional size
            _this.regex = /^([0-9xX]{4,6}|(?:x|X|\d\d?)(?:[-\. ](?:x|X|\d\d?)){3,5})(?:\s*\[([T\d]+)\])?(?:\s*(\d+))?/g;

            //                      |Chord name                 |pos < 10   |pos > 10, needs seperator                       | Optional fingerings|Optional size
            _this.searchRegex = /\b([ABCDEFG](?:[a-z0-9#])*)\s*\(?([0-9xX]{4,6}|(?:x|X|\d\d?)(?:[-\. ](?:x|X|\d\d?)){3,5})\)?(?:\s*\[([T\d]+)\])?(?:\s*(\d+))?/g;
        ...
        _createClass(ChordComponent, [{
            ...
            key: 'getDiagram',
            value: function getDiagram(scale, renderer) {
    ...
    exports.default = ChordComponent;
});